...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($columnType && ($columnType->data_type === 'json' || $columnType->data_type === 'jsonb')) {
            ...
            // Vérifier si la colonne role_id existe, sinon utiliser role
            ...
            if ($hasRoleId) {
                ...
                // Cas où role (varchar) existe, on doit d'abord créer role_id
                // Cette migration ne gère que la conversion de permission_id
                // La conversion de role vers role_id doit être faite par une autre migration
                DB::statement('
                    DO $$
                    DECLARE
                        rec RECORD;
                    ...
                    BEGIN
                        ...
                        FOR rec IN SELECT * FROM role_permissions LOOP
                            IF jsonb_typeof(rec.permission_id::jsonb) = \'array\' THEN
                                ...
                                FOR perm_id IN
                                    ...
                                    WHERE jsonb_typeof(value) = \'number\'
                                LOOP
                                    INSERT INTO temp_role_permissions (role, permission_id, created_at, updated_at)
                                    VALUES (rec.role, perm_id, rec.created_at, rec.updated_at);
                                END LOOP;
                            ELSIF jsonb_typeof(rec.permission_id::jsonb) = \'number\' THEN
                                -- Si c\'est un nombre simple
                                INSERT INTO temp_role_permissions (role, permission_id, created_at, updated_at)
                                VALUES (rec.role, (rec.permission_id::jsonb)::text::bigint, rec.created_at, rec.updated_at);
                            ELSIF rec.permission_id::text ~ \'^[0-9]+$\' THEN
                                -- Si c\'est une chaîne numérique
                                INSERT INTO temp_role_permissions (role, permission_id, created_at, updated_at)
                                VALUES (rec.role, (rec.permission_id::text)::bigint, rec.created_at, rec.updated_at);
                            END IF;
                        END LOOP;
                        ...
                        SELECT role, permission_id, created_at, updated_at
                        FROM temp_role_permissions;
            ...
            // Recréer la contrainte de clé étrangère pour role_id seulement si la colonne existe
            if ($hasRoleId) {
                DB::statement('
                    DO $$
                    BEGIN
