...
@include('platform-admin.layouts.menu')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Données globales /</span> Tout l'historique de balance
    </h4>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">Tout l'historique de balance</h5>
        ...
        <div class="card-body">
            ...
            <form method="GET" action="{{ route('platform-admin.global-data.historique-balance') }}" class="mb-4">
                <div class="row g-3">
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        <a href="{{ route('platform-admin.global-data.historique-balance') }}" class="btn btn-secondary w-100">Réinitialiser</a>
                    </div>
                </div>
                @if(request('per_page'))
                    ...
                    <tbody class="table-border-bottom-0">
                        @forelse($historiques as $historique)
                            <tr>
                                ...
                                <td>
                                    <strong>{{ $historique->balanceMarchand && $historique->balanceMarchand->entreprise ? $historique->balanceMarchand->entreprise->name : 'N/A' }}</strong>
                                </td>
                                <td>
                                ...
                                <td>
                                    @if($historique->createdBy)
                                        <small>{{ $historique->createdBy->first_name ?? '' }} {{ $historique->createdBy->last_name ?? '' }}</small>
                                    @else
