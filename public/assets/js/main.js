...
if (typeof $ !== 'undefined') {
  $(function () {
    ...
    if (searchInput.length) {
      ...
      // Search API AJAX call pour les colis
      var searchColis = function(query, callback) {
        $.ajax({
          url: '/api/search-colis',
          data: {
            q: query,
          ...
          success: function(data) {
            ...
            console.log('Colis count:', data.colis ? data.colis.length : 0);
            // Retourner juste les codes des colis pour tester
            var colisCodes = (data.colis || []).map(function(colis) {
              return colis.code;
            });
            console.log('Colis codes:', colisCodes);
            callback(colisCodes);
          },
          error: function(xhr, status, error) {
            console.error('Erreur recherche colis:', xhr.responseText, error);
            callback([]);
          }
        });
      };
          ...
          .typeahead(
            ...
            // Recherche de colis
            {
              ...
              source: function(query, callback) {
                console.log('Typeahead source called with query:', query);
                searchColis(query, function(colis) {
                  console.log('Typeahead callback received:', colis);
                  callback(colis);
                });
              },
              sorter: function(items) {
                // Ne pas trier, retourner tel quel
                return items;
              },
              matcher: function(item) {
                // Toujours retourner true pour afficher tous les résultats
                return true;
              },
              templates: {
                header: '<h6 class="suggestions-header text-primary mb-0 mx-4 mt-3 pb-2">Colis</h6>',
                suggestion: function (code) {
                  console.log('Suggestion function called with:', code);
                  return '<div class="suggestion-item">' + code + '</div>';
                },
                notFound:
                  '<div class="not-found px-4 py-2">' +
                  '<h6 class="suggestions-header text-primary mb-2">Colis</h6>' +
                  '<p class="py-2 mb-0"><i class="ti ti-alert-circle ti-xs me-2"></i> Aucun colis trouvé</p>' +
                  '</div>'
              }
            }
          )
